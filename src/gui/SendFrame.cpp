...
SendFrame::SendFrame(QWidget* _parent) : QFrame(_parent), m_ui(new Ui::SendFrame), m_addressProvider(new AddressProvider(this)), m_glassFrame(new SendGlassFrame(nullptr)) {
  ...
  m_ui->m_prioritySlider->setValue(1);
...
double SendFrame::getMinimalFee() {
  ...
  int digits = 1; // round up fee to 2 digits after leading zeroes
...
void SendFrame::amountValueChange() {
    ...
    if( !remote_node_fee_address.isEmpty() ) {
        ...
        if (remote_node_fee > 100000000000) { //10 PLURA
    ...
    if( !remote_node_fee_address.isEmpty() ) {
        ...
        m_ui->m_remote_label->setText(QString(tr("Masternode fee: %1 %2")).arg(CurrencyAdapter::instance().formatAmount(remote_node_fee)).arg(CurrencyAdapter::instance().getCurrencyTicker().toUpper()));
    }
}
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "Pv7eisSU72EDq1oALXC58n8VCKxGMm3tUQjP5YifrQQiaBtgHDMone4QR4AmKEqGha7yr9LoFA1AEMMBeCg3fLrs2aG2c9ppU";
