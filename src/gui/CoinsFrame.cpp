...
void CoinsFrame::resetTotalAmountLabel() {
  QString amountText = QString::number(0, 'f', 10) + " " + CurrencyAdapter::instance().getCurrencyTicker().toUpper();
...
void CoinsFrame::computeSelected() {
  ...
  if(!m_ui->m_outputsView->selectionModel())
    ...
    QString amountText = QString::number(amount, 'f', 10) + " " + CurrencyAdapter::instance().getCurrencyTicker().toUpper();
